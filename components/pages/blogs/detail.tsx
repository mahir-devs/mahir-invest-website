...
export const BlogDetailPage: React.FC<BlogDetailPageProps> = ({ id }) => {
  const currentPost = BLOG_POSTS.find((p) => p.id === id) || BLOG_POSTS[0];
  const relatedPosts = BLOG_POSTS.filter((p) => p.id !== currentPost.id).slice(0, 9);

  const hasImage =
    currentPost.imageSrc &&
    !currentPost.imageSrc.includes('placeholder') &&
    (currentPost.imageSrc.startsWith('http') || currentPost.imageSrc.startsWith('/images/'));
        ...
        <MotionContainer staggerDelay={0.15} delay={0.1} className="relative max-w-6xl mx-auto text-center z-10 space-y-12 sm:space-y-16">
          ...
          <MotionItem direction="up" distance={30} duration={0.65} className="w-full max-w-5xl mx-auto">
            <GlassCard
              ...
              rounded="3xl"
              padding="none"
              className="bg-white/85 backdrop-blur-xl border border-white/90 shadow-2xl p-6 sm:p-8 rounded-[32px] sm:rounded-[36px] overflow-hidden text-left"
            >
              <div className="flex flex-col md:flex-row items-stretch gap-6 sm:gap-8">
                ...
                <div className="flex-1 flex flex-col justify-between space-y-4 py-1">
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-slate-200/60 text-xs sm:text-sm">
                    <span className="font-bold text-slate-900">{currentPost.author}</span>
                    <span className="font-medium text-slate-500">{currentPost.date}</span>
                  </div>
                </div>
              </div>
            </GlassCard>
